...
class ActiveSupport::TestCase
  ...
  parallelize_setup do |worker|
    # Attachment tests create files, so each worker needs an isolated directory
    # to avoid moving files generated by other workers.
    $redmine_tmp_attachments_directory =
      File.join($redmine_tmp_attachments_directory, worker.to_s)
    FileUtils.mkdir_p $redmine_tmp_attachments_directory

    # Thumbnail tests assert file counts, so each worker needs an isolated
    # directory to avoid observing files generated by other workers.
