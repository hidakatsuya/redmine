...
class Gantts::ChartHelperTest < Redmine::HelperTest
  ...
  test 'builds scale links and labels from semantic segments' do
    gantt = stub(:chart => stub(:date_from => Date.new(2026, 1, 1)),
                 :params => {:controller => 'gantts', :action => 'show', :zoom => 2})
    month = Segment.new(:layer => 0, :label => '2026-2', :start_offset => 31, :span => 28,
  ...
  test 'builds subject DOM identifiers, row styles, and progress states' do
    ...
    assert_include 'id="issue-1"', gantt_row_subject_tag(stub(:has_children => false, :record => record, :subject => 'Subject'),
                                                           'Subject', [], [])
  end
  ...
  test 'renders issue selected column content and keeps other row kinds empty' do
    ...
    assert_include 'gantt__cell-value subject', gantt_row_column_content(stub(:issue? => true, :record => issue), column)
  end
  ...
  test 'builds chart styles including selected column dimensions' do
    chart = stub(:selected_columns => [stub, stub], :row_height => 32, :header_layers => 2, :day_width => 4,
                 :sidebar_subject_width => 330, :timeline_width => 120, :relations => [])
    query = stub(:draw_selected_columns => true, :draw_relations => false, :draw_progress_line => true)

    html = gantt_chart_tag(query, chart) { 'chart' }
