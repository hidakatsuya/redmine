...
class Redmine::Gantt::RowsTest < ActiveSupport::TestCase
  ...
  test 'version row retains completion and closed semantics and is frozen' do
    ...
    assert_equal version, row.version
    assert row.closed?
  ...
  test 'captures row state at build time' do
    ...
    assert_not row.behind_schedule?
  end
  ...
  test 'captures project and version status at build time' do
    ...
    assert_not project_row.overdue?
    assert_not version_row.closed?
    assert_not version_row.overdue?
    assert_not version_row.behind_schedule?
  end
