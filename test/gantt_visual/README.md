...
画面画像は現在のスクロール位置を基本画像として取得する。タイムラインの端の確認が必要なだけの
横スクロール量がある場合に限り、基本画像と異なる位置の`timeline-start`または`timeline-end`を
追加で取得する。短い期間や縮小表示で期間全体が収まる場合、および基本画像がすでに端にある場合は、
同じ位置の画像を重複して生成しない。
